...
export function IntroStage({ onScrollComplete, children }: IntroStageProps) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          const progress = Math.max(0, Math.min(1, -rect.top / (stageHeight - windowHeight)));
          
          // Cover mapping: starts at 10%, completes at 55%
          const coverStart = 0.10;
          const coverEnd = 0.55;
          const newCover = Math.max(0, Math.min(1, (progress - coverStart) / (coverEnd - coverStart)));
    ...
    return () => {
      ...
      window.removeEventListener('wheel', handleWheelCapture, { capture: true } as unknown as EventListenerOptions);
    };
  ...
  return (
    ...
    >
      ...
      <div className="title-layer" style={{ transform: `translateY(${titleTransform}px)` }}>
        <h1 className="site-title" aria-label="MYSXAN">
          {Array.from('MYSXAN').map((ch, idx) => (
            <span className="letter" style={{ '--delay': `${0.6 + idx * 0.06}s` } as React.CSSProperties} key={idx}>
              <span>{ch}</span>
            </span>
          ))}
        </h1>
